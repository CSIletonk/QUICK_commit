...
async function afficherCartes() {
    // Ajout de ? apres le liens pour les pararmetres
    let url = "https://api.scryfall.com/cards/search?q=e:ltr%20lang:fr&format=json&order=set&unique=prints"
    let hasMore = true;  // Pour la boucle dus a la pagination de lapi
    let template = document.querySelector("#card-template");
    list_symb = await getSymbols(); // Cherche les cartes puis les symboles

    while (hasMore) {
        let reponse = await fetch(url);
        let donnees = await reponse.json(); 

        // apres les promèses on cherches les données
        ...
        for (card of donnees.data) {
            ...
            for (info_mana of symb_mana) {
                let img = document.createElement("img"); // Crée l'image
